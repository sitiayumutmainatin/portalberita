...
class Tulisan extends CI_Controller
{
	...
	function simpan_tulisan()
	{
		...
		if (!empty($_FILES['filefoto']['name'])) {
			...
			if ($this->upload->do_upload('filepdf')) {
				...
				$this->m_tulisan->simpan_tulisan($judul, $isi, $kategori_id, $kategori_nama, $user_id, $user_nama, $gambar, $filepdf, $slug);
	...
	function update_tulisan()
	{
		...
		if (!empty($_FILES['filefoto']['name'])) {
			...
			if ($this->upload->do_upload('filepdf')) {
				...
				$user_id = $p['pengguna_id'];
				$user_nama = $p['pengguna_nama'];
				$this->m_tulisan->update_tulisan($tulisan_id, $judul, $isi, $kategori_id, $kategori_nama, $user_id, $user_nama, $gambar, $filepdf, $slug);
	...
	function viewpdf($filepdf)
	{
		if (isset($_POST['v_tulisan'])) {
			header("content-type: application/pdf");
			readfile('./assets/images/' . $filepdf . '.pdf');
		}
	}
	...
	function file()
	{
		...
		$tofile = realpath("./assets/images/" . $fname);
		header('Content-Type: application/pdf');
		readfile($tofile);
	}
}
